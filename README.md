# [QuantZone] Market Making Strategy for OrbDex Perpetuals
...
```
docker build -t orbdex-market-maker .
```
...
```
conda create -n orbdex-mm python=3.14
conda activate orbdex-mm
```
...
To authenticate with OrbDex, you need to provide your Wallet address and private key. You can get more information in the [OrbDex API documentation](https://docs.orbdex.trade/developer-portal/general-information/api-quick-start).

You can then set the `ORBDEX_L1_ADDRESS` and `ORBDEX_PRIVATE_KEY` environment variables. See below for more details. You will also need to set the `ORBDEX_ENVIRONMENT` environment variable to `mainnet` or `testnet`.

Example:
```
export ORBDEX_L1_ADDRESS="0x1234567890123456789012345678901234567890"
export ORBDEX_PRIVATE_KEY="0x1234567890123456789012345678901234567890123456789012345678901234"
export ORBDEX_ENVIRONMENT="testnet"
```
...
```
docker run -e PARAM_NAME=value orbdex-market-maker
```
