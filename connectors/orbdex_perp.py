"""
This module implements a connector for the OrbDex perpetual exchange.

It provides functionality to interact with the OrbDex API, including
order management, market data subscriptions, and account information.

Classes:
    OrbDexPerpConnector: Main connector class for OrbDex perpetual exchange.
"""
...
from orbdex_py.api.ws_client import OrbDexWebsocketChannel
from orbdex_py.common.order import Order as OrbDexOrder, OrderSide as OrbDexOrderSide, OrderType as OrbDexOrderType
from orbdex_py.environment import PROD, TESTNET
...
class OrbDexPerpConnector(ConnectorBase):
    """
    A connector class for interacting with the OrbDex perpetual exchange.

    This class provides methods for order management, market data subscriptions,
    and account information retrieval.

    Attributes:
        orbdex (OrbDex): The OrbDex client instance.
        exchange (str): The name of the exchange ("orbdex_perp").
    ...
    Methods:
        ...
        initialize(): Initialize the OrbDex client.
    ...
    def __init__(self, loop, key=None, secret=None):
        """
        Initialize the OrbDexPerpConnector.
        ...
        self.orbdex: OrbDex = None
        self.exchange = "orbdex_perp"

        self.l1_address = key or os.getenv("ORBDEX_L1_ADDRESS")
        self.l2_private_key = secret or os.getenv("ORBDEX_PRIVATE_KEY")
        ...
        self.rate_limiter = SyncRateLimiter(int(os.getenv("ORBDEX_RATE_LIMIT", "8"))) # X requests per second, adjust as needed

    def rate_limited_request(self, method: Callable, *args, **kwargs) -> Any:
        """
        Rate-limit the request to the OrbDex API.
        """
    ...
    def get_account_str(self):
        ...
        return str(self.orbdex.account.l2_address)

    async def initialize(self):
        """Initialize the OrbDex client with environment-specific settings."""
        _para_env = os.getenv("ORBDEX_ENVIRONMENT")
        _orbdex_perpetual_chain = PROD if _para_env == "mainnet" else (TESTNET if _para_env == "testnet" else _para_env)

        self.orbdex = OrbDex(
            env=_orbdex_perpetual_chain, 
    ...
    def cancel_order(self, client_order_id, by="exchange_order_id"):
        ...
        """            
        cancel_via_client_order_id = self.orbdex.api_client.cancel_order_by_client_id
        if by == 'client_order_id':
            ...
            cancel_method = self.orbdex.api_client.cancel_order
    ...
    def sync_open_orders(self, mkt):
        ...
        existing_orders = []
        try:
            p_orders = self.rate_limited_request(self.orbdex.api_client.fetch_orders, {'market': mkt})
        ...
        for ao in set(self.active_orders.keys()) - set(already_processed):
            try:
                po = self.rate_limited_request(self.orbdex.api_client.fetch_order_by_client_id, ao)
                self.process_order_update(po)
            except ValidationError as ve:
                if ve.data['message'] == 'rate limit exceeded':
                    self.logger.warning(f"rate limit exceeded")
    ...
    def cancel_all_orders(self, mkt):
        ...
        Args:
            mkt (str): The market symbol.
        """
        self.rate_limited_request(self.orbdex.api_client.cancel_all_orders, {"market":mkt})
    ...
    def insert_order(self, order: Order):
        ...
        if order.order_type == OrderType.LIMIT_MAKER:
            _order_type = OrbDexOrderType.Limit
            _order_instruction = "POST_ONLY"
        elif order.order_type == OrderType.LIMIT:
            _order_type = OrbDexOrderType.Limit
            _order_instruction = "GTC"
        elif order.order_type == OrderType.MARKET:
            _order_type = OrbDexOrderType.Market
            _order_instruction = "IOC"
        elif order.order_type == OrderType.IOC:
            _order_type = OrbDexOrderType.Limit
            _order_instruction = "IOC"

        _order_side = OrbDexOrderSide.Buy if order.side == Side.BUY else OrbDexOrderSide.Sell
        ...
        try:
            resp = self.rate_limited_request(self.orbdex.api_client.submit_order, _po)
        except Exception as e:
    ...
    async def _connect_to_ws(self):
        ...
        while not is_connected:
            is_connected = await self.orbdex.ws_client.connect()
            if not is_connected:
    ...
    def fetch_bbo(self, symbol: str) -> Dict[str, Level]:
        bbos = self.orbdex.api_client.fetch_bbo(symbol)
        return {
    ...
    async def subscribe_to_data_channels(self, market, callback):
        ...
        await self.orbdex.ws_client.subscribe(
            OrbDexWebsocketChannel.BBO,
            callback=self._on_bbo,
            params={"market": market},
        )

        await self.orbdex.ws_client.subscribe(
            OrbDexWebsocketChannel.FILLS,
            callback=self._on_fills,
            params={"market": market},
        )

        await self.orbdex.ws_client.subscribe(
            OrbDexWebsocketChannel.ORDER_BOOK_DELTAS,
            callback=self._on_orderbook,
            params={"market": market},
        )

        await self.orbdex.ws_client.subscribe(
            OrbDexWebsocketChannel.FUNDING_DATA,
    ...
    async def subscribe_to_trade_channels(self, market, callback):
        ...
        await self.orbdex.ws_client.subscribe(
            OrbDexWebsocketChannel.ORDERS,
            callback=self._on_order_update,
            params={"market": market},
        )

        await self.orbdex.ws_client.subscribe(
            OrbDexWebsocketChannel.TRADES,
    ...
    async def subscribe_to_account_channels(self, callback: Callable = None):
        ...
        await self.orbdex.ws_client.subscribe(
            OrbDexWebsocketChannel.ACCOUNT,
            self._on_account_update,
        )
        
        await self.orbdex.ws_client.subscribe(
            OrbDexWebsocketChannel.POSITIONS, 
            callback=self._on_positions
        )
        
        await self.orbdex.ws_client.subscribe(
            OrbDexWebsocketChannel.TRADEBUSTS, 
    ...
    async def setup_trading_rules(self, market):
        ...
        Args:
            market (str): The market symbol.
        """
        _para_markets = self.orbdex.api_client.fetch_markets()
    ...
    async def snapshots(self):
        ...
        positions = self.orbdex.api_client.fetch_positions()
