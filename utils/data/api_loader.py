...
from orbdex_py.environment import PROD
...
@dataclass
class DataConfig:
    """Configuration for OrbDex data operations"""
    client: OrbDexApiClient = OrbDexApiClient(env=PROD)
    base_dir: Path = Path("data/orbdex")
...
def retry_fetch(func: Callable) -> Callable:
    """Wrapper to add exponential backoff retries for OrbDex fetch API calls"""
...
# Patch OrbDexApiClient fetch methods with retries
OrbDexApiClient.fetch_trades = retry_fetch(OrbDexApiClient.fetch_trades)
OrbDexApiClient.fetch_markets = retry_fetch(OrbDexApiClient.fetch_markets)
OrbDexApiClient.fetch_markets_summary = retry_fetch(OrbDexApiClient.fetch_markets_summary)

def load_trades_market(market: str, start_at: str = None, end_at: str = None, data_config: DataConfig = DataConfig()):
    """Load trades for a specific market from OrbDex API and save to parquet files"""
...
def load_trades(data_config: DataConfig = DataConfig()):
    """Load all trades from OrbDex API for each market"""
    data_config.mkdirs()
...
def load_markets_summary(data_config: DataConfig = DataConfig()):
    """Load market summary data for all available markets from OrbDex API"""
    data_config.mkdirs()
    markets = duckdb.query(f"select symbol from '{data_config.base_dir}/markets.parquet' order by symbol").df()
    for market in markets['symbol']:
...
def load_markets(data_config: DataConfig = DataConfig()):
    """Load markets data from OrbDex API and save to parquet file"""
