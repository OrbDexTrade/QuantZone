...
from orbdex_py.common.order import (
    Order as OrbDexOrderObject,
    OrderSide as OrbDexOrderSide,
    OrderType as OrbDexOrderType,
)

from hummingbot.connector.constants import s_decimal_NaN
from hummingbot.connector.derivative.orbdex_perpetual import (
    orbdex_perpetual_constants as CONSTANTS,
    orbdex_perpetual_web_utils as web_utils,
)
from hummingbot.connector.derivative.orbdex_perpetual.orbdex_perpetual_api_order_book_data_source import (
    OrbDexPerpetualAPIOrderBookDataSource,
)
from hummingbot.connector.derivative.orbdex_perpetual.orbdex_perpetual_auth import OrbDexPerpetualAuth
from hummingbot.connector.derivative.orbdex_perpetual.orbdex_perpetual_user_stream_data_source import (
    OrbDexPerpetualUserStreamDataSource,
)
...
class OrbDexPerpetualDerivative(PerpetualDerivativePyBase):
    web_utils = web_utils
    ...
    def __init__(
        ...
        self.orbdex_perpetual_l1_address = orbdex_perpetual_l1_address
        self.orbdex_perpetual_is_testnet = orbdex_perpetual_is_testnet
        self.orbdex_perpetual_l1_private_key = orbdex_perpetual_l1_private_key
        self.orbdex_perpetual_l2_private_key = orbdex_perpetual_l2_private_key
    ...
    @property
    def name(self) -> str:
        # Note: domain here refers to the entire exchange name. i.e. orbdex_perpetual or orbdex_perpetual_testnet
        return self._domain

    @property
    def authenticator(self) -> OrbDexPerpetualAuth:
        return OrbDexPerpetualAuth(
                self.orbdex_perpetual_l1_address,
                self.orbdex_perpetual_is_testnet,
                self.orbdex_perpetual_l1_private_key,
                self.orbdex_perpetual_l2_private_key)
    ...
    async def _initialize_trading_pair_symbol_map(self):
        try:
            self.logger().info("Retrieving trading pairs from OrbDex Perpetuals...")
            exchange_info = await self._api_get(path_url=self.trading_pairs_request_path,
                                                 data={})

            self.logger().info("got trading pairs from OrbDex Perpetuals...")
            self._initialize_trading_pair_symbols_from_exchange_info(exchange_info=exchange_info)
        except Exception:
            self.logger().exception("There was an error requesting exchange info.")

    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return OrbDexPerpetualAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return OrbDexPerpetualUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            connector=self,
            api_factory=self._web_assistants_factory,
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        self.logger().debug(f"The order {order_id} does not exist on OrbDex Perpetuals. "
    ...
    async def _place_order(
        ...
        param_order_type = OrbDexOrderType.Limit
        param_order_tif = "GTC"
        if order_type == OrderType.LIMIT_MAKER:
            param_order_tif = "POST_ONLY"
        elif order_type == OrderType.MARKET:
            param_order_type = OrbDexOrderType.Market

        side_map = {
            TradeType.BUY: OrbDexOrderSide.Buy,
            TradeType.SELL: OrbDexOrderSide.Sell
        }

        self.logger().info(f"Creating order tif: {param_order_tif}")
        order_object = OrbDexOrderObject(
            stp="EXPIRE_BOTH",
            client_id=order_id,
            market=symbol,
        ...
        order_object.signature = self._auth.orbdex_account.sign_order(order_object)
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from OrbDex. Check API key and network connection.",
                )
    ...
    async def _trading_pair_position_mode_set(self, mode: PositionMode, trading_pair: str) -> Tuple[bool, str]:
        ...
        if initial_mode != mode:
            msg = "OrbDex only supports the ONEWAY position mode."
            success = False
        else:
            ...
            self.logger().debug(f"OrbDex switching position mode to " f"{mode} for {trading_pair} succeeded.")
        return success, msg

    async def _set_trading_pair_leverage(self, trading_pair: str, leverage: int) -> Tuple[bool, str]:
        # TODO
        msg = "Leverage setting is not supported by OrbDex."
        success = True
        return success, msg
